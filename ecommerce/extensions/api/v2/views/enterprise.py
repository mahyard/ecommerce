...
class EnterpriseCouponViewSet(CouponViewSet):
    ...
    def _create_offer_assignment_email_sent_record(self, enterprise_customer, email_type, template_id=None):
        OfferAssignmentEmailSentRecord.create_email_record(enterprise_customer, email_type, template_id)
    ...
    def assign(self, request, pk):  # pylint: disable=unused-argument
        ...
        if serializer.is_valid():
            ...
            self._create_offer_assignment_email_sent_record(enterprise_customer, ASSIGN, template_id)
            return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def revoke(self, request, pk):  # pylint: disable=unused-argument
        ...
        if serializer.is_valid():
            serializer.save()
            # Create a record of the email sent
            self._create_offer_assignment_email_sent_record(enterprise_customer, REVOKE, template_id)
            return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def remind(self, request, pk):  # pylint: disable=unused-argument
        ...
        if serializer.is_valid():
            serializer.save()
            # Create a record of the email sent
            self._create_offer_assignment_email_sent_record(enterprise_customer, REMIND, template_id)
            return Response(serializer.data, status=status.HTTP_200_OK)
